...
<?php
    ...
    $passage = "";
    while (strlen($passage) < 10000) {
        $randnum = mt_rand(0, 100);
        if ($randnum < 50) {
            $randbosh = $bosh[array_rand($bosh, 1)];
            $randbosh = str_replace("x", $theme, $randbosh);
            $passage = substr_replace($passage, $randbosh, strlen($passage));
        } else {
            $randfamous = $famous[array_rand($famous, 1)];
            $randfamous = str_replace("a", $before[array_rand($before, 1)], $randfamous);
            $randfamous = str_replace("b", $after[array_rand($after, 1)], $randfamous);
            $passage = substr_replace($passage, $randfamous, strlen($passage));
        }
    }
    echo $passage;
?>
