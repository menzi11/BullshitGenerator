...
<?php
    ...
    $文章 = "";
    while (strlen($文章) < 10000) {
        $随机数 = mt_rand(0, 100);
        if ($随机数 < 50) {
            $随机论述 = $论述[array_rand($论述, 1)];
            $随机论述 = str_replace("主题", $主题, $随机论述);
            $文章 = substr_replace($文章, $随机论述, strlen($文章));
        } else {
            $随机名人名言 = $名人名言[array_rand($名人名言, 1)];
            $随机名人名言 = str_replace("曾经说过", $前面垫话[array_rand($前面垫话, 1)], $随机名人名言);
            $随机名人名言 = str_replace("这不禁令我深思", $后面垫话[array_rand($后面垫话, 1)], $随机名人名言);
            $文章 = substr_replace($文章, $随机名人名言, strlen($文章));
        }
    }
    echo $文章;
?>
